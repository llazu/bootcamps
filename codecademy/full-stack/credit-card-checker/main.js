...
const validateCred = (arr) => {
    ...
    const sum = myArr.reduce( (acc, pos) => acc + pos);
    return sum % 10 === 0;
}
...
const idInvalidCardCompanies = (arr) => {
    let myArr = [];
    const invalid = findInvalidCard(arr);
    for (let i = 0; i < invalid.length; i++) {
        if (invalid[i][0] === 3) {
            myArr.push('Amex (American Express)')
        } else if (invalid[i][0] === 4) {
            myArr.push('Visa')
        } else if (invalid[i][0] === 5) {
            myArr.push('Mastercard');
